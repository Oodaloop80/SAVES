...
step ca root homelab-root-ca.crt
```
...
openssl req -new -sha256 \
  -key forgejo.key \
  -out forgejo.csr \
  -subj "/C=US/O=Homelab/CN=forgejo.lan"
```
...
openssl verify -CAfile homelab-root-ca.crt forgejo.crt
```
...
```bash
cp homelab-root-ca.crt /volume1/docker/homelab-root-ca.crt
```
...
scp your_admin_user@192.168.1.201:/root/ca/homelab-root-ca.key .
scp your_admin_user@192.168.1.201:/root/ca/homelab-root-ca.crt .
```

Then on the NAS:

```bash
shred -u /root/ca/homelab-root-ca.key
```
...
- **step-ca path:** run `step ca root homelab-root-ca.crt` on the workstation (Phase 3A.5). **Distribute the root, not `intermediate_ca.crt`** — installing the intermediate as a trust anchor appears to work in some clients and fails in others, which is a miserable bug to chase.
- **openssl fallback path:** `homelab-root-ca.crt` from Phase 3.8/3.9.

### Windows

```powershell
# Run PowerShell as Administrator
Import-Certificate -FilePath "C:\path\to\homelab-root-ca.crt" `
  -CertStoreLocation Cert:\LocalMachine\Root
```

Or GUI: double-click the `.crt` → **Install Certificate** → **Local Machine** → **Place all certificates in the following store** → **Trusted Root Certification Authorities**.
...
```bash
sudo cp homelab-root-ca.crt /etc/pki/ca-trust/source/anchors/
...
```bash
git config --global http.sslCAInfo "C:/path/to/homelab-root-ca.crt"
```
...
export NODE_EXTRA_CA_CERTS=/etc/ssl/certs/homelab-root-ca.crt

# Windows PowerShell profile
$env:NODE_EXTRA_CA_CERTS = "C:\certs\homelab-root-ca.crt"
```
...
### Browser: `NET::ERR_CERT_AUTHORITY_INVALID` after Phase 11

The CA went into the wrong store, or you're using Firefox — **Firefox maintains its own certificate store and ignores the OS.** Import via Settings → Privacy & Security → Certificates → View Certificates → Authorities → Import.

### Browser: `NET::ERR_CERT_COMMON_NAME_INVALID`

The address you typed isn't in the cert's SAN list. Re-issue from Phase 3.4 with the right name.
...
4. Give it the CA cert so it can validate TLS — mount `homelab-root-ca.crt` into the runner container
...
`CLAUDE.md` → *Git Workflow*; the constraining decision is recorded in `docs/ROADMAP.md` →
*Decisions locked*.

**⚠️ This forge is now the only copy of that history.** With GitHub gone there is no off-site
remote. `/volume1/docker/forgejo` — the repo tree **and** a `pg_dump` of the database — must be
in the backup set (see *Backups* above). Local clones hold the commits but not issues, PRs, or
settings.
...
| Memory | 2 GB + 1 GB (Postgres) | `SAVES_MEM_LIMIT`, default 3 GB |
| CPU quota | ❌ impossible (§6) | ❌ same — never add `cpus:` |
| Runs as | UID 1030:65536, `cap_drop: ALL` | root in-container (not yet hardened — see below) |

The **`cpus:` finding (§6) and the top-level-`version:`-key finding** were both propagated into
`docker/docker-compose.yml`; `scripts/preflight_nas.sh` check `[6]` now fails the deploy if
either regresses, and reports RAM headroom against the limits other containers already reserve.

**The NAS pulls SAVES from the NAS.** `git pull` in `/volume1/docker/saves/app` talks to this
forge over the loopback LAN address, so the forge must be up to *update* SAVES. A running
`saves_app` container needs no git access at all, so a forge outage never interrupts saving.
The clone needs the step-ca root in the NAS trust store plus a `repository: Read` PAT —
sequence in `docs/PROD_ROLLOUT.md` step 1.

**Not yet applied to SAVES:** the hardening in this document (non-root UID, `cap_drop: ALL`,
`read_only`, `no-new-privileges`) is Forgejo-only. `saves_app` still runs as root in-container
and writes to a bind-mounted vault. Bringing it up to the same standard is a sensible follow-on
once PROD is stable — it is *not* a prerequisite for the rollout.
...
3. **Container identity:** `user: "1030:65536"` — UID 1030 with the *`service accounts`* GID, deliberately **not** DSM's broad `users` GID 100.
